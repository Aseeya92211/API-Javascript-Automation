...
df = pd.read_csv("NSDC_Production_API_Testing_Report.csv")
...
try:
    ...
    if data["Total Failure Validations"] == [0]:
        with open("prod_API_Report.html", 'w') as file:
            file.write('<center>'
                        + '<h2><b> NSDC Production Environment API Testing Report </b></h1><br><hr>'
                        + '<h3><b> Summary </b></h3>'
